...
Last updated: 2026-02-13T16:15:00+04:00
Owner: Arafa
...
| TASK-PLAN.5: Fix Mood Feed Status Trigger | DONE | Migration 194: Found mood row already existed in `feed_status_live` (from migration 162 line 78-80) — task description's assumption was wrong. Actual gap: trigger was INSERT-only while newer triggers (supplements/bjj/screen_time) fire on INSERT OR UPDATE. Fix: ensured row exists (ON CONFLICT DO NOTHING safety net), recreated trigger with AFTER INSERT OR UPDATE. Test: inserted test mood → status `unknown`→`ok`, updated mood → `events_today` incremented to 2. Cleaned up test row. Down migration tested (reverts to INSERT-only) and re-applied. Smoke tests: 8/8 healthy. 2 files created. Commit `2b30772`. |
| TASK-PLAN.6: Create Habits Replay Test | DONE | Created `ops/test/replay/habits.sh` (147 LOC) following `calendar.sh` multi-check pattern. 4 checks: active-habits (active=5, total=5), completions-freshness (age=3.7h, rows=2, latest=2026-02-13), dashboard-payload (habits_today array with 5 habits), streak-function (5 habits with streaks, max_streak=2). JSON output validated. `all.sh` auto-discovers via glob — 5 domains now tested (calendar, finance, habits, health, nutrition). Habits: PASS (all 4 checks ok). 1 file created. |
