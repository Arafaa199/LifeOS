-- Migration 126 DOWN: Restore immutability triggers, remove unique date indexes,
-- restore PIPE.1 trigger functions (nested BEGIN/EXCEPTION pattern).
-- Note: Deleted duplicate rows cannot be restored.

BEGIN;

-- Remove unique date indexes, restore non-unique DESC indexes
...
-- Restore immutability triggers on raw.whoop_* tables
...
-- Restore PIPE.1 trigger functions (nested BEGIN/EXCEPTION, conflict on cycle_id/sleep_id/strain_id)
...
BEGIN
    ...
    IF v_raw_id IS NULL THEN
        SELECT id INTO v_raw_id FROM raw.whoop_cycles WHERE date = NEW.date ORDER BY id DESC LIMIT 1;
    END IF;
...
BEGIN
    ...
    IF v_raw_id IS NULL THEN
        SELECT id INTO v_raw_id FROM raw.whoop_sleep WHERE date = NEW.date ORDER BY id DESC LIMIT 1;
    END IF;
...
BEGIN
    ...
    IF v_raw_id IS NULL THEN
        SELECT id INTO v_raw_id FROM raw.whoop_strain WHERE date = NEW.date ORDER BY id DESC LIMIT 1;
    END IF;
