-- Migration 138: Fix weight source priority
--
-- Problem: life.refresh_daily_facts() takes the LATEST weight entry by timestamp,
-- regardless of source. This causes stale test/webhook data to overwrite real
-- ha_eufy_scale readings.
--
-- Example (2026-02-03):
-- | recorded_at | value  | source        |
-- |-------------|--------|---------------|
-- | 17:53:20    | 109.90 | ha_eufy_scale | ← REAL from HA
-- | 18:19:10    | 111.55 | webhook       | ← STALE test data (incorrectly wins!)
--
-- Fix: Prefer ha_eufy_scale > ios-app > others, then by timestamp within each tier.
-- Also: Clean up known test data sources.

BEGIN;

-- =============================================================================
-- 1. Clean up stale test data from health.metrics
-- =============================================================================

DELETE FROM health.metrics
WHERE metric_type = 'weight'
  AND source IN ('webhook', 'test', 'external_test', 'pivpn_direct', 'ha_manual_test', 'ha_direct_test');

-- =============================================================================
-- 2. Rewrite life.refresh_daily_facts() with weight source priority
-- =============================================================================
...
BEGIN
    ...
    BEGIN
        ...
        SELECT
            ...
            -- Priority: HealthKit first, then health.metrics with source preference
        ...
        FROM
            ...
            -- Weight: legacy health.metrics with SOURCE PRIORITY (migration 138 fix)
            -- Prefers ha_eufy_scale (real scale) over ios-app over test/webhook sources
            LEFT JOIN LATERAL (
                SELECT value AS weight_kg
                FROM health.metrics
...
'Single-pipeline refresh with nutrition + weight source priority. Reads directly from raw layer + nutrition:
- raw.whoop_cycles (recovery)
- raw.whoop_sleep (sleep, ms→min conversion)
- raw.whoop_strain (strain + calories_active)
- raw.healthkit_samples + health.metrics (weight, with ha_eufy_scale priority)
...
Migration 138: Fixed weight source priority (ha_eufy_scale > ios-app > others).';

-- =============================================================================
-- 3. Refresh today to apply the fix
-- =============================================================================

SELECT * FROM life.refresh_daily_facts(life.dubai_today(), 'migration_138_weight_fix');
