...
-- Migration 173: Fix weight fallback in dashboard.get_payload()
-- Problem: When a new day starts, weight shows as NULL or a stale value (111.55kg)
-- because webhook entries from HealthKit sync push stale data.
-- Solution:
--   1. Block webhook source for weight metrics (only ha_eufy_scale and ios-app are valid)
--   2. Add COALESCE fallback in get_payload() to most recent known weight
--   3. Fix mood summary view column names

-- =============================================================================
-- 1. Block webhook weight entries at database level
-- =============================================================================

-- Delete all existing webhook weight entries (stale HealthKit data)
DELETE FROM health.metrics WHERE metric_type = 'weight' AND source = 'webhook';

-- Create trigger to reject future webhook weight entries
CREATE OR REPLACE FUNCTION health.reject_webhook_weight()
RETURNS TRIGGER AS $$
BEGIN
    IF NEW.metric_type = 'weight' AND NEW.source = 'webhook' THEN
        RAISE NOTICE 'Rejecting webhook weight entry (use ha_eufy_scale or ios-app instead)';
...
DROP TRIGGER IF EXISTS trg_reject_webhook_weight ON health.metrics;
CREATE TRIGGER trg_reject_webhook_weight
    BEFORE INSERT ON health.metrics
    FOR EACH ROW
    EXECUTE FUNCTION health.reject_webhook_weight();

-- =============================================================================
-- 2. Fix mood summary view (column names mismatch)
-- =============================================================================
...
-- =============================================================================
-- 3. Update dashboard.get_payload() with weight fallback
-- =============================================================================
...
BEGIN
    ...
    -- Build final result (schema_version 16: Fixed weight fallback)
    result := jsonb_build_object(
        'schema_version', 16, 'generated_at', NOW(), 'target_date', the_date,
        'today_facts', jsonb_build_object(
            ...
            -- FIX: Use today's weight OR fallback to latest known weight
...
COMMENT ON FUNCTION dashboard.get_payload IS 'Dashboard payload v16: Fixed weight fallback to latest known value when today has no weight';

-- =============================================================================
-- 4. Refresh today to clear any stale weight
-- =============================================================================

SELECT * FROM life.refresh_daily_facts(life.dubai_today(), 'migration_173_weight_fix');

-- Track migration
INSERT INTO ops.schema_migrations (filename)
VALUES ('173_fix_dashboard_weight_fallback.up.sql')
