-- Migration 124: Fix WHOOP propagation triggers to fire on UPDATE (not just INSERT)
-- Problem: n8n does UPSERT (ON CONFLICT date DO UPDATE) on legacy tables.
-- After initial INSERT fires the trigger, subsequent UPDATEs are invisible to normalized layer.
-- Fix 1: Recreate triggers with AFTER INSERT OR UPDATE.
-- Fix 2: Rewrite trigger functions so normalized updates succeed even when raw.* immutability blocks raw updates.
--         Uses nested BEGIN/EXCEPTION blocks: raw insert failure is caught independently, normalized insert always runs.

BEGIN;

-- ============================================================
-- 1. Rewrite trigger functions with independent raw/normalized error handling
-- ============================================================
...
DECLARE
    v_raw_id bigint;
BEGIN
    -- Try raw insert (may fail on UPDATE due to raw.whoop_cycles immutability trigger)
    BEGIN
    ...
    EXCEPTION WHEN OTHERS THEN
        -- Raw immutability blocked the update — look up existing raw_id
        SELECT id INTO v_raw_id FROM raw.whoop_cycles WHERE cycle_id = NEW.id;
    END;

    -- Always update normalized (independent of raw outcome)
...
DECLARE
    v_raw_id bigint;
BEGIN
    -- Try raw insert (may fail on UPDATE due to raw.whoop_sleep immutability trigger)
    BEGIN
        ...
        RETURNING id INTO v_raw_id;
    EXCEPTION WHEN OTHERS THEN
        SELECT id INTO v_raw_id FROM raw.whoop_sleep WHERE sleep_id = NEW.id;
    END;

    -- Always update normalized
...
DECLARE
    v_raw_id bigint;
BEGIN
    -- Try raw insert (may fail on UPDATE due to raw.whoop_strain immutability trigger)
    BEGIN
        ...
        RETURNING id INTO v_raw_id;
    EXCEPTION WHEN OTHERS THEN
        SELECT id INTO v_raw_id FROM raw.whoop_strain WHERE strain_id = NEW.id;
    END;
...
-- ============================================================
-- 2. Recreate triggers with AFTER INSERT OR UPDATE
-- ============================================================
