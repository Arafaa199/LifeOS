...
-- Migration 158: Add music listening summary to dashboard payload
-- Creates view for daily music stats and wires into dashboard.get_payload()

-- 1. Create daily music summary view (Dubai timezone)
CREATE OR REPLACE VIEW life.v_daily_music_summary AS
SELECT
    (started_at AT TIME ZONE 'Asia/Dubai')::date AS day,
    COUNT(*) AS tracks_played,
    ROUND(COALESCE(SUM(duration_sec), 0) / 60.0, 1) AS total_minutes,
    COUNT(DISTINCT artist) FILTER (WHERE artist IS NOT NULL) AS unique_artists,
    (ARRAY_AGG(artist ORDER BY artist) FILTER (WHERE artist IS NOT NULL))[1] AS top_artist,
    (ARRAY_AGG(album ORDER BY album) FILTER (WHERE album IS NOT NULL))[1] AS top_album
FROM life.listening_events
GROUP BY (started_at AT TIME ZONE 'Asia/Dubai')::date;

-- Actually compute top_artist by frequency, not alphabetical
DROP VIEW IF EXISTS life.v_daily_music_summary;

CREATE VIEW life.v_daily_music_summary AS
WITH daily AS (
    SELECT
        (started_at AT TIME ZONE 'Asia/Dubai')::date AS day,
        artist,
...
top_artists AS (
    SELECT
        ...
        artist,
        COUNT(*) AS play_count,
        ROW_NUMBER() OVER (PARTITION BY day ORDER BY COUNT(*) DESC, artist) AS rn
    FROM daily
...
top_albums AS (
    SELECT
        ...
        album,
        COUNT(*) AS play_count,
        ROW_NUMBER() OVER (PARTITION BY day ORDER BY COUNT(*) DESC, album) AS rn
    FROM daily
...
SELECT
    d.day,
    COUNT(*) AS tracks_played,
    ROUND(COALESCE(SUM(d.duration_sec), 0) / 60.0, 1) AS total_minutes,
    COUNT(DISTINCT d.artist) FILTER (WHERE d.artist IS NOT NULL) AS unique_artists,
    ta.artist AS top_artist,
    tb.album AS top_album
FROM daily d
LEFT JOIN top_artists ta ON ta.day = d.day AND ta.rn = 1
LEFT JOIN top_albums tb ON tb.day = d.day AND tb.rn = 1
GROUP BY d.day, ta.artist, tb.album;

-- 2. Rewrite dashboard.get_payload() to include music_today (schema v14)
...
BEGIN
    ...
    -- Build final result (schema_version 14: Added music_today)
    result := jsonb_build_object(
        'schema_version', 14, 'generated_at', NOW(), 'target_date', the_date,
...
VALUES ('158_music_dashboard.up.sql')
