-- Migration 130: Exclude non-economic transactions from aggregation views
-- Transfers, CC Payments, and BNPL Repayments are not real spending/income.
-- They move money between accounts or repay debt — they should not inflate totals.

BEGIN;

-- 1. normalized.v_daily_finance — exclude non-economic categories from spend/income
...
-- 2. finance.v_dashboard_finance_summary — exclude non-economic from month totals
...
-- 3. finance.mv_monthly_spend — exclude BNPL Repayment from spending
...
-- 4. finance.mv_spending_anomalies — exclude BNPL Repayment
...
WITH category_baselines AS (
    ...
    WHERE t.amount < 0
      ...
      AND t.category NOT IN ('Transfer', 'CC Payment', 'BNPL Repayment', 'Credit Card Payment')
)
...
-- 5. finance.mv_category_velocity — exclude BNPL Repayment
