-- Rollback: Remove timeline from life.get_daily_summary()
-- Task: TASK-VIS.2

-- Drop enhanced function
DROP FUNCTION IF EXISTS life.get_daily_summary(DATE);

-- Restore original function (without timeline)
...
BEGIN
    ...
    -- Finance section (without timeline)
    SELECT jsonb_build_object(
...
COMMENT ON FUNCTION life.get_daily_summary(DATE) IS 'Returns complete daily summary (rollback from TASK-VIS.2)';
