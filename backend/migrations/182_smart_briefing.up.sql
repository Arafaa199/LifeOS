...
-- Migration 182: Rewrite life.explain_today() with priority-ranked smart briefing
--
-- Synthesizes richer daily_facts data: 7-day trends for recovery/sleep/spend,
-- BJJ integration (streak, training nudges, recovery context), sleep composition
-- (deep sleep, efficiency), spend anomalies (1.5x 7-day avg), nutrition flags
-- (protein < 120g, unusual calories), mood/energy acknowledgment.
--
-- Outputs 3-5 sentences max, sorted by priority. Skips normal metrics.
-- Same return shape as v1 — dashboard.get_payload() unchanged.
...
COMMENT ON FUNCTION life.explain_today IS 'v2: Priority-ranked coach briefing — 7-day trends, BJJ integration, sleep composition, spend anomalies (1.5x), nutrition, mood/energy. 3-5 sentences max.';

-- Update migration tracking (replace v2 entry if exists)
DELETE FROM ops.schema_migrations WHERE filename IN ('182_explain_today_v2.up.sql', '182_smart_briefing.up.sql');
INSERT INTO ops.schema_migrations (filename)
VALUES ('182_smart_briefing.up.sql')
