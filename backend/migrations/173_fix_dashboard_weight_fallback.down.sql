...
-- Rollback migration 173

-- Drop the webhook weight rejection trigger
DROP TRIGGER IF EXISTS trg_reject_webhook_weight ON health.metrics;
DROP FUNCTION IF EXISTS health.reject_webhook_weight();

-- Restore original mood summary view (aggregated version)
...
-- Restore previous get_payload without weight fallback (schema v15)
...
BEGIN
    ...
    SELECT COALESCE(
        (SELECT jsonb_build_object(
            ...
            'log_count', log_count
        ) FROM raw.v_daily_mood_summary WHERE date = the_date),
        NULL
    ) INTO mood_data;

    result := jsonb_build_object(
        'schema_version', 15, 'generated_at', NOW(), 'target_date', the_date,
...
DELETE FROM ops.schema_migrations WHERE filename = '173_fix_dashboard_weight_fallback.up.sql';
